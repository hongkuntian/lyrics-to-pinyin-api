...
test('song library contract carries normalized rows and hidden performer turns',()=>{
  ...
  const document=makeDocument({catalog_id:'123',artist:'Test artist',title:'Original test song',duration:12},response,'current');
  const payload={version:1,state:'ready',document};
  assert.deepEqual(validateSchema(schema,payload),[]);
  const malformed=structuredClone(payload);malformed.document.structure.sourceRows[0].timestamp='2';
