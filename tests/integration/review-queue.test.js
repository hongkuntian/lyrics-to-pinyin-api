...
async function fixture(t,{report=true,second=false}={}) {
  ...
  const provider={uploads:0,submits:0,gets:0,remote:null,text:'',
    ...
    async submit(local){this.submits++;this.remote={id:'batch-test',status:'in_progress',endpoint:'/v1/responses',input_file_id:local.input_file_id,
      metadata:{lyra_batch_id:local.id,request_hash:local.request_hash,policy_version:REVIEW_POLICY}};return this.remote;},
...
async function complete(f,decision='keep') {
  const items=(await f.db.query('SELECT * FROM correction_batch_items ORDER BY operation_id')).rows;
...
test('reordered completed results settle once, release unused verification, and never publish',async t=> {
  ...
  assert.equal((await f.db.query("SELECT count(*) FROM correction_review_queue WHERE state='assessed'")).rows[0].count,2);
  assert.equal((await f.db.query("SELECT count(*) FROM correction_reports WHERE status='pending'")).rows[0].count,2);
