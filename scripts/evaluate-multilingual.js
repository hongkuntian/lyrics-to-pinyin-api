...
export async function evaluate({apiKey,corpus,translate=generate,explain=generateExplanation,log=console.log}={}) {
  ...
  const reserve=jobs.reduce((n,j)=>n+reservationMicros(j.translationRequest)+reservationMicros(j.studyRequest),600_000);
  if(jobs.length!==12||reserve>2_000_000)throw new Error('evaluation_budget_exceeded');
    ...
    while(index<jobs.length) {
      ...
      try {
        ...
        if(item.language==='en') {
          ...
          const request=explanationBody(doc,accepted,translatedSelection,'en');
          if(reservationMicros(request)>200_000)throw new Error('translated_study_budget_exceeded');
          const result=await explain(doc,accepted,translatedSelection,{apiKey,explanationLanguage:'en',generationRequest:request});
          translatedStudy={selection:word.segment,explanation:result.content,responseID:result.response?.id,usageMicros:result.actualMicros};
        }
        log(JSON.stringify({translatedStudy,event:'multilingual_evaluation_case',case:item.id,direction:`${item.language}:${target}`,checks:item.checks,source:item.lines,translation:translated.content,selection:selection.text,explanation:explanation.content,usageMicros:(translated.actualMicros??0)+(explanation.actualMicros??0),translationResponseID:translated.response?.id,explanationResponseID:explanation.response?.id}));
      } catch(error) {
        failed++;
...
export async function runEvaluation(env=process.env) {
  ...
  await evaluate({apiKey:env.OPENAI_API_KEY,corpus});
}
