...
export async function reserveExplanation(database,{key,doc,translation,selection,recipe,userID,amount}) {
  return database.transaction(async db=>{
    ...
    const head=await first(db,`SELECT h.revision_id FROM translation_heads h JOIN song_translations t ON t.id=h.translation_id WHERE t.document_id=$1 AND t.target='en' FOR UPDATE OF t`,[doc.id]);
    ...
    const id=randomUUID();
    const row=await first(db,`INSERT INTO study_explanations(id,cache_key,document_id,revision_id,source_id,lower_offset,upper_offset,recipe,user_id,state)
      VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9,'queued') RETURNING *`,[id,key,doc.id,translation.id,selection.sourceID,selection.lower,selection.upper,recipe,userID]);
